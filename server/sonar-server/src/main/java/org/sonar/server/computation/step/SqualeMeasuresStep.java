...
public class SqualeMeasuresStep implements ComputationStep {
  private final TreeRootHolder treeRootHolder;
  private final MetricRepository metricRepository;
  private final MeasureRepository measureRepository;
  private final SqaleRatingSettings sqaleRatingSettings;

  public SqualeMeasuresStep(TreeRootHolder treeRootHolder, MetricRepository metricRepository, MeasureRepository measureRepository,
  ...
  private class SqualeMeasuresVisitor extends PathAwareVisitor<DevelopmentCost> {
    ...
    public void visitFile(Component file, Path<DevelopmentCost> path) {
      if (!file.getFileAttributes().isUnitTest()) {
        path.current().add(computeDevelopmentCost(file));
    ...
    private void saveDebtRatioMeasure(Component component, double density) {
      measureRepository.add(component, debtRatioMetric, newMeasureBuilder().create(density));
    }
  ...
  private static long getLongValue(Optional<Measure> measure) {
    ...
    if (measure.get().getValueType() == Measure.ValueType.LONG) {
      return measure.get().getLongValue();
    }
    return 0L;
  }
